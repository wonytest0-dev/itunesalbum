...
function matchQuery(item, query, exact) {
  ...
  const itemFullText = normalize(item.fullText);

  const queryArtist = normalize(query.artist);
  const queryTitle = normalize(query.title);
  const queryFullText = normalize(`${query.artist} - ${query.title}`);

  if (exact) {
    return (
      itemArtist === queryArtist &&
      itemTitle === queryTitle
    ) || itemFullText === queryFullText;
  }

  return (
    itemArtist.includes(queryArtist) &&
    itemTitle.includes(queryTitle)
  ) || itemFullText.includes(queryArtist) && itemFullText.includes(queryTitle);
}
